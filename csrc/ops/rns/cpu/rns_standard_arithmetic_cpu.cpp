...
void standard_binary_loop(torch::Tensor out,
...
void check_cpu_standard_binary(const torch::Tensor& out,
                               const torch::Tensor& lhs,
                               const torch::Tensor& rhs,
                               const torch::Tensor& params,
...
void standard_binary_into(torch::Tensor out,
  ...
  check_cpu_standard_binary(
      out_rows, lhs_rows, rhs_rows, params, operation_name);
  AT_DISPATCH_INTEGRAL_TYPES(lhs.scalar_type(), "rns_standard_binary_cpu", [&] {
    standard_binary_loop<scalar_t, operation>(
...
void rns_add_standard_inplace_cpu(torch::Tensor lhs,
                                  const torch::Tensor rhs,
                                  const torch::Tensor rns_params) {
  standard_binary_into<StandardBinaryOperation::kAdd>(
...
torch::Tensor rns_sub_standard_cpu(const torch::Tensor lhs,
                                   const torch::Tensor rhs,
                                   const torch::Tensor rns_params) {
  auto out = torch::empty_like(lhs);
...
torch::Tensor rns_montgomery_mul_row_scalars_standard_cpu(
    const torch::Tensor residues,
    const torch::Tensor row_scalars,
    const torch::Tensor rns_params) {
  constexpr const char* operation = "rns_montgomery_mul_row_scalars_standard";
          ...
          while (index < end) {
            ...
            for (int64_t coefficient = coefficient_begin;
              ...
              row_result[coefficient * result_stride2] =
                  fhelium::cpu::reduce_to_standard(
...
void rns_sub_standard_inplace_cpu(torch::Tensor lhs,
                                  const torch::Tensor rhs,
                                  const torch::Tensor rns_params) {
  standard_binary_into<StandardBinaryOperation::kSubtract>(
...
TORCH_LIBRARY_IMPL(fhelium_rns_ops, CPU, m) {
  ...
  m.impl("add_standard_", &rns_add_standard_inplace_cpu);
  m.impl("sub_standard", &rns_sub_standard_cpu);
