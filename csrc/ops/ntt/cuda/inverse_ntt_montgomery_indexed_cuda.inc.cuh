...
// Public transform-only indexed radix-2 INTT entry points.

template <typename scalar_t>
void launch_inverse_ntt_montgomery_indexed_cuda(
    torch::Tensor a,
...
void inverse_ntt_montgomery_indexed_inplace_cuda(
  ...
  AT_DISPATCH_INTEGRAL_TYPES(
      a.scalar_type(), "inverse_ntt_montgomery_indexed_cuda", ([&] {
        launch_inverse_ntt_montgomery_indexed_cuda<scalar_t>(batch_rows,
                                                             even_indices,
                                                             odd_indices,
                                                             inverse_twiddles,
                                                             rns_params);
