...
def ntt_reuse(
    ...
    accumulator = None
    reference = torch.zeros_like(fixed_values)
    for index in range(3):
        source_values = small_complex_vector(
            engine.num_slots, seed=301 + index, scale=0.005
        )
        source = engine.coefficient_domain_to_ntt_domain(
            engine.encrypt_message(source_values)
        )
        # The same transformed fixed operand participates in every product.
        product = engine.multiply(source, fixed)
        accumulator = (
            product if accumulator is None else engine.add(accumulator, product)
        )
        reference += source_values * fixed_values
