...
Run on one process or one process per GPU:
    ...
    python examples/22_distributed_partial_results.py

    torchrun --standalone --nproc-per-node=2 \
        examples/22_distributed_partial_results.py --size 8
    ...
    try:
        ...
        for rotation_step in local_rotation_steps:
            ...
            diagonal = engine.prepare_plaintext_for_multiplication(
                engine.encode(
                    _cyclic_diagonal_slots(
                        matrix, rotation_step, engine.num_slots
                    ),
                    depth=rotated.depth,
                )
            )
