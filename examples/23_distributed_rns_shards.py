...
2. rank 0 prepares NTT form and scatters its limbs; ranks multiply locally;
   rank 0 reconstructs, relinearizes, and rescales the full result.
...
def main() -> None:
    ...
    parser.add_argument(
        "--preset",
        type=parse_preset,
        default=fh.Preset.slots32768_scale40_depth34_int64,
    )
    ...
    try:
        ...
        local_a = dist.scatter_ciphertext_limbs(
            ciphertext_a,
            limb_ranges=addition_ranges if dist.get_rank() == 0 else None,
            src=0,
        )
        ...
        # This schedule gathers and prepares NTT form on rank 0. NTT is also
        # row-local, so another schedule could keep these shards distributed.
        ...
        local_operand = dist.scatter_ciphertext_limbs(
            prepared_sum,
            limb_ranges=multiplication_ranges if dist.get_rank() == 0 else None,
            src=0,
        )
        ...
        if dist.get_rank() == 0:
            ...
            expected = (message_a + message_b).square().cpu()
