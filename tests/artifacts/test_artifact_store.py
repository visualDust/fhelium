...
from fhelium.values import Ciphertext, Plaintext, SecretKey, TensorResident
...
def test_artifact_lifecycle_preserves_one_current_generation(
    ...
    assert (
        original.artifact_schema_version
        == artifact_store_module.ARTIFACT_SCHEMA_VERSION
    )
...
def test_store_version_and_unrecognized_nonempty_roots_fail_closed(
    ...
    unsupported_version = artifact_store_module.STORE_SCHEMA_VERSION + 1
    ...
    with pytest.raises(UnsupportedArtifactStoreVersionError) as unsupported:
        ArtifactStore(versioned_root)
    assert unsupported.value.found_version == unsupported_version
    assert unsupported.value.supported_versions == (
        artifact_store_module.STORE_SCHEMA_VERSION,
    )
...
def test_catalog_identity_and_schema_fail_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="normalized UUID"):
        ArtifactStore(identity_root)
