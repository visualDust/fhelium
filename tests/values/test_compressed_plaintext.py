...
def test_strided_sparse_compressed_plaintext_is_not_a_multiply_operand(
    ...
    unique_count = 32
    compact = prepared.data[..., :unique_count].clone()
    implicit = prepared.data[..., unique_count].clone()
    dense_data = implicit[..., None].expand_as(prepared.data).clone()
    ...
    dense = fh.Plaintext(
        ...
        data=dense_data,
        representation="rns",
        polynomial_domain=prepared.polynomial_domain,
        modulus_basis=prepared.modulus_basis,
        residue_representation=prepared.residue_representation,
        prime_ids=prepared.prime_ids,
    )
    compressed = fh.CompressedPlaintext.from_plaintext(
        dense,
        unique_count=unique_count,
    ...
    with pytest.raises(ValueError, match="multiplication requires"):
        engine.multiply_plaintext(ciphertext, compressed)


@pytest.mark.gpu
@pytest.mark.parametrize("compression_layout", ["cyclic", "contiguous"])
@pytest.mark.parametrize("operation", ["add", "multiply"])
def test_batched_ciphertext_compressed_arithmetic_matches_dense_exactly(
    engine: Engine,
    compression_layout: fh.CompressedPlaintextLayout,
    operation: str,
) -> None:
    ...
    message = torch.stack(
        (0.01 * torch.sin(index * 0.01), 0.02 * torch.cos(index * 0.02))
    )
    factor = 0.005 * torch.cos(index * 0.03)
    ciphertext = engine.encrypt_message(message)
    if operation == "multiply":
        ciphertext = engine.coefficient_domain_to_ntt_domain(ciphertext)
        seed = engine.prepare_plaintext_for_multiplication(
            engine.encode(factor)
        )
        method = engine.multiply_plaintext
        inplace_method = engine.multiply_plaintext_
    else:
        seed = engine.prepare_plaintext_for_addition(engine.encode(factor))
        method = engine.add_plaintext
        inplace_method = engine.add_plaintext_
    dense, compressed = _with_repeated_encoded_axis(
        seed,
        unique_count=32,
        compression_layout=compression_layout,
    ...
    expected = method(ciphertext, dense)
    actual = method(ciphertext, compressed)
    assert torch.equal(actual.data, expected.data)

    target = ciphertext.clone()
    returned = inplace_method(target, compressed)
...
def test_real_periodic_slots_compress_and_multiply_numerically(
    ...
    max_error = torch.max(torch.abs(decoded - (message * factor).cpu())).item()
