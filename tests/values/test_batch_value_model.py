...
def test_multidimensional_batch_public_facades_preserve_shape(batch_engine):
    base = _messages(torch.get_default_device())
...
def test_batched_ntt_matches_independent_single_value_calls(batch_engine):
    messages = _messages(torch.get_default_device())
...
def test_batch_mismatch_rejection(batch_engine):
    messages = _messages(torch.get_default_device())
    ciphertext = batch_engine.encrypt_message(messages)
    with pytest.raises(RuntimeError, match="operand batch counts differ"):
        batch_engine.add(ciphertext, ciphertext.select_batch(0))

    ntt = batch_engine.coefficient_domain_to_ntt_domain(ciphertext)
    with pytest.raises(RuntimeError, match="exactly two components"):
    ...
    broadcast = batch_engine.add_plaintext(
        ciphertext,
        singleton_batched_plaintext,
    )
    torch.testing.assert_close(
        batch_engine.decrypt_message(
            broadcast,
            batch_engine.secret_key.to("cpu"),
            is_real=True,
            device="cpu",
        )[..., : messages.size(-1)],
        (messages + messages[:1]).cpu(),
        atol=1e-5,
        rtol=0,
    )

    singleton_batched_factor = (
        batch_engine.prepare_plaintext_for_multiplication(
            batch_engine.encode(messages[:1])
        )
    )
    product = batch_engine.ntt_domain_to_coefficient_domain(
        batch_engine.multiply_plaintext(ntt, singleton_batched_factor)
    )
    torch.testing.assert_close(
        batch_engine.decrypt_message(
            product,
