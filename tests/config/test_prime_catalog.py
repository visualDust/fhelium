...
def test_config_dump_records_version_and_exact_moduli() -> None:
    ...
    assert dumped["q_moduli"] == list(config.q_moduli)
    assert dumped["p_moduli"] == list(config.p_moduli)
    assert dumped["galois_generator"] == 5
    assert CkksConfig.parse(dumped).dumps() == dumped


def test_config_dump_rejects_another_package_version_or_prime_selection() -> (
    None
):
    config = CkksConfig.parse(Preset.slots8192_scale40_levels7_int64)
    wrong_version = config.dumps()
    wrong_version["fhelium_version"] = "incompatible-test-version"
    with pytest.raises(ValueError, match="requires FHElium"):
        CkksConfig.parse(wrong_version)

    wrong_primes = config.dumps()
    q_moduli = list(config.q_moduli)
    q_moduli[0] += 2
    wrong_primes["q_moduli"] = q_moduli
    with pytest.raises(ValueError, match="Q moduli"):
        CkksConfig.parse(wrong_primes)
