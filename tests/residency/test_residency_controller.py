...
def test_search_limit_reports_inconclusive_with_exact_evidence() -> None:
    ...
    request = _request("search-limit", requested_handle)

    limited_search = ResidencyController(manager, search_state_limit=1)
    with pytest.raises(ResidencySearchLimitError) as error_info:
        limited_search.decide(request)
    assert error_info.value.request_name == "search-limit"
