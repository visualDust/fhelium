...
`CompressedPlaintext` is an exact operation-ready RNS value whose last axis can be reconstructed without loss from repeated values. Its implementation defines exact-value metadata, reconstruction rules, dispatcher boundaries, and native arithmetic paths.
...
Compression layout is exact value metadata. `CkksEngine` validates that
...
- **strided_sparse**: compact index `u` occupies dense index `u * (N/U)`;
  other positions carry one stored exact implicit value per batch and limb.
...
dense plaintext. This preserves exact behavior for arbitrary implicit values
...
residue representation, and exact prime IDs. It is therefore serializable,
...
integer mode, and no kernel branches on representation. This makes the exact
...
encoding is strided sparse, and its NTT representation has `2r` exact values
...
usually destroys exact equality among NTT positions. The source message still
...
- Exact batch matching and the one unbatched-broadcast case are identical to
...
`Plaintext`, represent both arithmetic states as two exact values.

## Source map

| Responsibility | Source |
| --- | --- |
| Exact compressed value, layouts, checks, and expansion | `fhelium/core/compressed_plaintext.py` |
...
| Exact conversion and arithmetic tests | `tests/test_compressed_plaintext.py` |
