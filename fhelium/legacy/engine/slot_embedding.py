...
from fhelium.legacy.engine.galois import forward_slot_generator_positions
...
# convention rather than allowing independently configured engines to attach
...
def make_slot_tensor(
    ...
    """Materialize a dense ``[*batch, slot]`` message layout.
...
def encode_slots(
    ...
    r"""Encode ordered slots as stochastic-rounded integer coefficients.
...
def inverse_embed_slots(
    ...
    *,
    ...
    ``m`` is ``[*batch, slot]`` in FHElium's generator-3 order or the
...
def decode_slots(
    ...
    The result is complex binary64 ``[*batch, slot]`` with final
...
def embed_coefficients(
    ...
    *,
    ...
    and permute $z$ into the slot order selected by ``galois_generator``. PyTorch
...
def generator_permutation(
    ...
    The permutation is defined as mu_p(n) = pn mod M where p is coprime with
    M and p=2*k+1.
    """
    M = 2 * N
    ...
    pn = p * n % M
    if verbose:
        print(f"Generator permutation for p={p} is\n{pn}")
    return pn
...
def prepost_perms(
    ...
    generator_perm = generator_permutation(N, k=(galois_generator - 1) // 2)
    fold_perm = fold_permutation(N, generator_perm)
