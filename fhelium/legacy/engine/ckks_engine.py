...
from fhelium.legacy.engine.ckks_decryptor import CkksDecryptor
from fhelium.legacy.engine.ckks_encryptor import CkksEncryptor
from fhelium.legacy.engine.ckks_plaintext_codec import CkksPlaintextCodec
from fhelium.legacy.engine.ckks_rescale import CkksRescaler
from fhelium.legacy.engine.direct_keyswitch_consumer import (
    StandardDirectKeySwitchDigitConsumer,
)
from fhelium.legacy.engine.galois import (
    apply_coefficient_galois_automorphism,
    coefficient_galois_gather_indices,
    rotation_galois_element,
)
from fhelium.legacy.engine.hybrid_keyswitch import HybridKeySwitcher
...
class CkksEngine:
    ...
    def galois_generator(self) -> int:
        """Return the Galois generator selected by the CKKS configuration."""
    ...
    def create_rotation_key(
        ...
        For normalized signed step $r$, generation derives the Galois element $g$,
        ...
        return self._key_generator.create_rotation_key(
            self._normalize_rotation_step(rotation_step),
    ...
    def rotation_keys(self) -> RotationKeySet:
        """Mutable installed mapping from normalized signed steps to keys."""
    ...
    def rotation_key(self, rotation_step: int) -> RotationKey:
        ...
        rotation_step = self._normalize_rotation_step(rotation_step)
    ...
    def set_rotation_key(self, key: RotationKey) -> None:
        """Install a key under its self-described normalized rotation step."""
    ...
    def relinearize(
        ...
        d0 = d0 + correction0
        d1 = d1 + correction1
        self.rns_runtime.reduce_to_standard_(d0)
    ...
    def rotate_by_step(
        ...
        The step is reduced modulo $S$ into the signed representative range. A matching direct key is used
        ...
        normalized_step = self._normalize_rotation_step(int(rotation_step))
        path = self._rotation_path_for_step(normalized_step, {})
    ...
    def rotate_with_key(
        ...
        r"""Rotate slots using the normalized signed step carried by ``key``.
        ...
        rotation_step = self._normalize_rotation_step(key.rotation_step)
        if rotation_step != key.rotation_step:
            raise ValueError(
                "Rotation key must carry a normalized signed rotation step: "
                f"key={key.rotation_step}, normalized={rotation_step}"
            )
        ...
        *,
        ...
        for rotation_step in rotation_steps:
            ...
            normalized_steps.append(
                self._normalize_rotation_step(int(rotation_step))
            )
        ...
        for index, (step, path) in enumerate(
            zip(normalized_steps, paths, strict=True)
        ):
            if len(path) <= 1:
    ...
    def rotate_many_with_keys(
        ...
        *,
        ...
        Every key self-describes its normalized signed step. Output order and
        ...
        for key in rotation_keys:
            ...
            if rotation_step != key.rotation_step:
                raise ValueError(
                    "Rotation key must carry a normalized signed rotation "
                    f"step: key={key.rotation_step}, normalized={rotation_step}"
                )
    ...
    def __str__(self) -> str:
        return (
            f"CkksEngine(id={self.id}, config={self.config}, "
    ...
    def validate_public_key(self, key: PublicKey) -> None:
        ...
        matching ring dimension, dtype, device, polynomial domain,
    ...
    def validate_key_switch_key(self, key: KeySwitchKey) -> None:
        ...
        The concrete key type is preserved while ring dimension,
        ...
        if isinstance(key, RotationKey):
            ...
            if key.rotation_step != normalized_step:
                raise ValueError(
                    "RotationKey rotation_step is not normalized for this "
                    f"engine: {key.rotation_step} != {normalized_step}"
                )
    ...
    def _normalize_rotation_step(self, rotation_step: int) -> int:
        return RotationKey.normalize_step(
            rotation_step,
            ring_dimension=self.config.N,
        )
    ...
    def _add_ciphertext_payloads(
        ...
        *,
        ...
        if result_batch is not None and rhs_batch is not None:
            self.rns_runtime.add_standard_(
                result_batch,
                rhs_batch,
    ...
    def _subtract_ciphertext_payloads(
        ...
        *,
        ...
        rhs_batch = self._flatten_component_batch(rhs.data)
        if result_batch is not None and rhs_batch is not None:
            self.rns_runtime.sub_standard_(
                result_batch,
                rhs_batch,
                prime_ids=result.prime_ids,
            )
    ...
    def _add_compressed_plaintext_component(
        ...
        *,
        ...
        Functional mode allocates non-aliasing standard residues in $[0,q_i)$;
    ...
    def _assert_rotation_key_for_step(
        ...
        """Validate one external key for an already normalized requested step."""
    ...
    def _engine_rotation_key(
        ...
        """Return one checked engine-owned key for a normalized direct step."""
    ...
    def _rotation_path_for_step(
        ...
        """Resolve an engine-owned key path for one normalized step."""
    ...
    def _apply_rotation_key(
        ...
        """Apply one normalized rotation key to an engine-local ciphertext."""
    ...
    def _apply_rotation_automorphism(
        ...
        *,
        ...
        standard residues in $[0,q_i)$, is newly allocated, and does not alias
