"""Render a selected mixed-level Program function as an SSA/dataflow SVG."""
...
class SvgGraphError(RuntimeError):
    """Report that Program SVG rendering could not complete."""


def _display_name(operation: Operation) -> str:
    """Return a stable local display name for an operation."""

    if operation.results and operation.results[0].name_hint:
        return f"%{operation.results[0].name_hint}"
    return operation_name(operation)


SvgGraphField = Literal[
    "name",  # Stable SSA result or block-argument name.
    "opcode",  # Registered xDSL operation name, such as fhelium_ckks.add.
...
class SvgNodeSection:
    ...
    ``value`` participates in Graphviz layout. ``tooltip`` optionally adds
    complete or alternate details for the containing node's aggregate hover
...
class SvgOperationContext:
    """Provide stable operation data to an SVG presentation policy."""
...
class _ImmutableColorMap(Mapping[str, str]):
    """Store an immutable color mapping for theme value semantics."""
...
class SvgGraphTheme:
    ...
    uses the default constant color, while an empty mapping removes it.
    ...
    def operation_fill_color(self, context: SvgOperationContext) -> str:
        """Return the configured or stable palette color."""
        ...
        configured = self.operation_colors.get(key)
...
class SvgGraphPresentation:
    """Select and format operation details for an SVG graph.
    ...
    ``operation_tooltip`` for aggregate hover details. These methods must not
    mutate their operation or Program.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(fields, (str, bytes)):
            raise TypeError("IR SVG fields must be a collection of field names")
        selected_fields = _ALL_FIELDS if fields is None else frozenset(fields)
        unknown_fields = selected_fields - _ALL_FIELDS
        if unknown_fields:
            raise ValueError(
                f"Unknown IR SVG visualization fields: {sorted(unknown_fields)}"
            )
        if not selected_fields:
            raise ValueError(
                "IR SVG visualization requires at least one displayed field"
            )
        selected_attributes: frozenset[str] | None = None
        if attribute_names is not None:
            if isinstance(attribute_names, (str, bytes)):
                raise TypeError(
                    "IR SVG attribute_names must be a collection of names"
                )
            ...
            if not all(
                ...
                raise ValueError(
                    "IR SVG attribute_names must contain non-empty strings"
                )
        if attribute_preview_chars is not None:
            if isinstance(attribute_preview_chars, bool) or not isinstance(
                ...
                raise TypeError(
                    "IR SVG attribute_preview_chars must be an integer or None"
                )
            if attribute_preview_chars <= 0:
                raise ValueError(
                    "IR SVG attribute_preview_chars must be positive"
                )
        if theme is None:
            theme = SvgGraphTheme()
        elif not isinstance(theme, SvgGraphTheme):
            raise TypeError("IR SVG theme must be SvgGraphTheme")
    ...
    def format_attribute_value(
        ...
        """Format one selected attribute for its character-limited preview."""
...
def _validated_operation_sections(
    ...
    if not isinstance(sections, tuple):
        raise TypeError(
            "IR SVG operation_sections must return a tuple of "
            "SvgNodeSection values"
        )
    if not all(isinstance(section, SvgNodeSection) for section in sections):
        raise TypeError(
            "IR SVG operation_sections must return only SvgNodeSection values"
        )
    return sections


@dataclass(frozen=True, init=False)
class _SvgGraphRenderer:
    """Render one selected entry's SSA/dataflow graph to an SVG file.
    ...
    Args:
        ...
        presentation: Operation-detail and theme policy. ``None`` constructs
            the default presentation.
        rank_direction: Graphviz rank direction: top-to-bottom, bottom-to-top,
            left-to-right, or right-to-left.
        name: Diagnostic renderer name.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if path.suffix.lower() != ".svg":
            raise ValueError("IR SVG output_path must end with '.svg'")
        if presentation is None:
            presentation = SvgGraphPresentation()
        elif not isinstance(presentation, SvgGraphPresentation):
            raise TypeError("IR SVG presentation must be SvgGraphPresentation")
        if rank_direction not in {"TB", "BT", "LR", "RL"}:
            raise ValueError(
                "IR SVG rank_direction must be 'TB', 'BT', 'LR', or 'RL'"
            )
    ...
    def render(self, program: Program) -> int:
        """Render one entry function and return its operation count."""

        path = self.output_path
        if path.exists() and not self.overwrite:
            raise SvgGraphError(
                f"IR SVG visualization would overwrite existing file {path}"
            )
        try:
            import pydot
        except ImportError as exc:
            raise SvgGraphError(
                "IR SVG visualization requires pydot>=4.0.1"
            ) from exc

        presentation = self.presentation
        theme = presentation.theme
        block = program.single_block(self.entry)
        operations = tuple(
            operation
        ...
        for index, operation in enumerate(operations):
            ...
            operation_context = SvgOperationContext(
                operation,
                tuple(value_names[result] for result in operation.results),
                tuple(value_names[operand] for operand in operation.operands),
            )
            sections = _validated_operation_sections(
                presentation, operation_context
            )
            tooltip = presentation.operation_tooltip(
                operation_context, sections
            )
            if tooltip is not None and not isinstance(tooltip, str):
                raise TypeError(
                    "IR SVG operation_tooltip must return a string or None"
                )
            node_attributes: dict[str, Any] = {
                "label": _record_label(sections),
                "shape": "record",
                "style": "filled,rounded",
                "fillcolor": theme.operation_fill_color(operation_context),
                "fontcolor": theme.node_font_color,
                "color": theme.node_stroke_color,
            }
        ...
        try:
            ...
            raise SvgGraphError(
                f"IR SVG visualization failed for {path}: {exc}"
            ) from exc
        if not path.is_file() or path.stat().st_size == 0:
            raise SvgGraphError(
                f"IR SVG visualization did not produce a file at {path}"
            )
...
class SvgGraphOutput:
    """Record the SVG file produced by a visualization pass."""
...
class SvgGraphVisualizationPass:
    """Render one Program function to a local or temporary SVG file.

    ``output_path`` selects a caller-owned local path. When it is omitted, the
    pass creates a unique directory under the operating system's temporary
    location. In both cases, ``SvgGraphOutput`` records the produced file in
    ...
    def run(
        ...
        path = self.output_path
        if path is None:
            directory = Path(tempfile.mkdtemp(prefix="fhelium-program-svg-"))
            path = directory / f"{self.entry}.svg"
        renderer = _SvgGraphRenderer(
            path,
        ...
        matched = renderer.render(program)
        output = SvgGraphOutput(path.resolve(), temporary)
        shared_data[SvgGraphOutput] = output
        return PassResult.unchanged(
            program,
            matched=matched,
            diagnostics=(f"wrote Program SVG to {output.path}",),
        )
