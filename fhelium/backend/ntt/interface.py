"""Protocol and table-row alignment for configured NTT schedule executors.

The :class:`NttBackend` protocol describes the representation transitions
requested by the RNS context. Schedule executors own family-specific device
tables and invoke the matching native schemas. ``NativeNttImplementation`` in
``fhelium.backend.ntt.operations`` implements NTT IR operations and delegates
these transitions through an RNS context and one schedule executor.
...
contiguous interval in the context's QP parameter order, and callers
supply that interval with ``parameter_row_start``. This prevents a partial
basis or key-switch digit from being matched to parameters merely because it
happens to have a particular row count.
"""
...
class NttBackend(Protocol):
    r"""Representation-specific NTT operations consumed by ``RnsContext``.
    ...
    interval in the backend's complete QP tables. Implementations
    ...
    semantic differences visible to ``RnsContext``.
    ...
    aligned with parameter row ``parameter_row_start + j`` and
    ...
    in $[0,2q_i)$ except ``inverse_to_standard_`` ($[0,q_i)$) and
...
def slice_ntt_parameter_rows(
    ...
    Args:
        ...
        twiddles: Complete backend table whose first dimension follows the
            context's QP prime-row order.
        rns_params: Complete RNS parameter tensor with shape
            ``[parameter, prime]`` in the same prime-row order.
        parameter_row_start: Zero-based parameter row corresponding to
            ``operand[..., 0, :]``.
    ...
    if row_stop > twiddles.size(0) or row_stop > rns_params.size(1):
        raise ValueError(
            "NTT operand rows exceed the prepared parameter range: "
