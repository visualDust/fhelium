...
class RnsLayout:
    ...
    def level_for_active_row_count(
        self,
        row_count: int,
        *,
        ...
        Complete Q and QP tensors drop one leading Q row per chain position,
        so their active row count identifies the corresponding position.  A
        partial digit or arbitrary prime interval is not a complete basis and
        must not use this lookup.
        """

        if type(row_count) is not int or row_count <= 0:
            raise ValueError("active RNS row count must be a positive integer")
        level = self.row_count(0, include_p=include_p) - row_count
        ...
        if self.row_count(level, include_p=include_p) != row_count:
            raise ValueError(
                "active RNS row count differs from the configured basis: "
                f"rows={row_count}, level={level}, include_p={include_p}"
            )
        return level
